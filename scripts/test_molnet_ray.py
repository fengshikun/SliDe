...
log_prefix = 'lmh'
cu_list = [0, 1, 2, 3]
pretrained_model = '/data/protein/SKData/DenoisingData/DeCL/pretraining_f2d_pos_frad_ctl_gf_frag_mask0.2_unipcq/step=225289-epoch=2-val_loss=0.3073-test_loss=0.2361-train_per_step=0.2483.ckpt'

task_cmd_lst = []
exe_cmd_lst = []
for cid in cu_list:
    task_cmd_lst.append(open(f'{cid}_mol_ray_cmd.sh', 'w'))
...
for task in all_tasks:
    ...
    test_cmd = f'CUDA_VISIBLE_DEVICES={device_id} python -u scripts/train_ray.py --conf examples/ET-Molnet.yaml --layernorm-on-vec whitened --job-id {log_prefix}_{task} --dataset-arg {task} --bond-length-scale 0 --denoising-weight 0.1 --test-interval 1 --lr-schedule cosine --lr-cosine-length {step_num} --num-steps {step_num} --seed {seed}  {cmd_suffix} --ray-result-file ./ray/{log_prefix}_{task}.txt'
