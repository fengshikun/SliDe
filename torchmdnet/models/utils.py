...
class GatedEquivariantBlockInject(nn.Module):
    """Gated Equivariant Block as defined in Schütt et al. (2021):
    Equivariant message passing for the prediction of tensorial properties and molecular spectra
    """
    ...
    def __init__(
        ...
        super(GatedEquivariantBlockInject, self).__init__()
        self.out_channels = out_channels
        ...
        act_class = act_class_mapping[activation]
        self.update_net = nn.Sequential(
            nn.Linear(hidden_channels * 2 + inject_dim, intermediate_channels),
            act_class(),
            nn.Linear(intermediate_channels, out_channels * 2),
        )
    ...
    def forward(self, x, v, inject_info, batch):
        vec1 = torch.norm(self.vec1_proj(v), dim=-2)
        vec2 = self.vec2_proj(v)

    
        inject_info = torch.tensor(inject_info, dtype=x.dtype, device=x.device)
        inject_info = inject_info[batch]
        x = torch.cat([x, vec1, inject_info], dim=-1)
        x, v = torch.split(self.update_net(x), self.out_channels, dim=-1)
        v = v.unsqueeze(1) * vec2
