...
class QM9A(QM9_geometric):
    def __init__(self, root, transform=None, dataset_arg=None, dihedral_angle_noise_scale=0.1, position_noise_scale=0.005, composition=False, transform_y=None, bat_noise=1, bond_var=0.004, angle_var=0.005, torsion_var_r=0.1, torsion_var=0.02, coord_var=0.004):
        assert dataset_arg is not None, (
            "Please pass the desired property to "
            'train on via "dataset_arg". Available '
            f'properties are {", ".join(qm9_target_dict.values())}.'
        ...
        self.bond_var = 0.004 # bond var
        self.angle_var = angle_var # angle var
        self.torsion_var_r = torsion_var_r # torsion var rotate
        self.torsion_var = torsion_var # torsion var
        self.coord_var = coord_var # coor var
    ...
    def __getitem__(self, idx: Union[int, np.integer, IndexType]) -> Union['Dataset', Data]:
        ...
        if self.bat_noise:
            new_mol, bond_label_lst, angle_label_lst, dihedral_label_lst, rotate_dihedral_label_lst = add_equi_noise_new(mol, bond_var=self.bond_var, angle_var=self.angle_var, torsion_var_r=self.torsion_var_r, torsion_var=self.torsion_var, coord_var=self.coord_var, add_ring_noise=True)

            pos_noise_coords = np.zeros((atom_num, 3), dtype=np.float32)
            coord_conf = new_mol.GetConformer() # .GetPositions()
            for i in range(atom_num):
                c_pos = coord_conf.GetAtomPosition(i)
                pos_noise_coords[i] = [float(c_pos.x), float(c_pos.y), float(c_pos.z)]
            
            
            org_data.pos_target = torch.tensor(pos_noise_coords - org_data.pos.numpy(), dtype=torch.float32)
            if torch.isnan(org_data.pos_target).sum():
                print(f'bat noise nan on idx {idx.item()}')
                return self.default_noise(org_data)
            
            org_data.pos = torch.tensor(pos_noise_coords, dtype=torch.float32)
            return org_data
        # pos_noise = np.zeros((atom_num, 3), dtype=np.float32)
