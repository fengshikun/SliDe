...
class MD17A(InMemoryDataset):
    ...
    def __init__(self, root, transform=None, pre_transform=None, dataset_arg=None, dihedral_angle_noise_scale=0.1, position_noise_scale=0.005, composition=False, reverse_half=False, addh=False, cod_denoise=False, bat_noise=0):
        assert dataset_arg is not None, (
            "Please provide the desired comma separated molecule(s) through"
            f"'dataset_arg'. Available molecules are {', '.join(MD17.available_molecules)} "
            "or 'all' to train on the combined dataset."
    ...
    def __getitem__(self, idx: Union[int, np.integer, IndexType]) -> Union['Dataset', Data]:
        ...
        if self.bat_noise:
            new_mol, bond_label_lst, angle_label_lst, dihedral_label_lst, rotate_dihedral_label_lst = add_equi_noise_new(mol, bond_var=0.001, angle_var=0.001, torsion_var_r=2, torsion_var=0.001, coord_var=0.001, add_ring_noise=True)
        else:
            org_angle = []
